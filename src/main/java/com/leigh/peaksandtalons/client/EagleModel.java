...
/** Eagle V2: a layered bald-eagle silhouette rather than the alpha box bird. */
public class EagleModel extends HierarchicalModel<EagleEntity> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
        ResourceLocation.fromNamespaceAndPath(PeaksAndTalons.MOD_ID, "eagle_v2"), "main");
    ...
    public EagleModel(ModelPart root) {
        this.root=root; body=root.getChild("body"); neck=body.getChild("neck"); head=neck.getChild("head");
        leftWing=body.getChild("left_wing"); rightWing=body.getChild("right_wing");
        leftForeWing=leftWing.getChild("forewing"); rightForeWing=rightWing.getChild("forewing");
        tail=body.getChild("tail"); leftLeg=body.getChild("left_leg"); rightLeg=body.getChild("right_leg");
    }
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        PartDefinition b=r.addOrReplaceChild("body", CubeListBuilder.create()
            .texOffs(0,0).addBox(-4.5F,-5F,-6F,9F,10F,13F)
            .texOffs(0,23).addBox(-3.5F,-6F,-4F,7F,3F,9F), PartPose.offset(0,14,1));
        ...
        t.addOrReplaceChild("tail_l", CubeListBuilder.create().texOffs(64,22).addBox(0,-.7F,0,4F,1.4F,8F), PartPose.offset(1.5F,0,5F));
        t.addOrReplaceChild("tail_r", CubeListBuilder.create().texOffs(64,22).mirror().addBox(-4F,-.7F,0,4F,1.4F,8F).mirror(false), PartPose.offset(-1.5F,0,5F));
        PartDefinition ll=b.addOrReplaceChild("left_leg", CubeListBuilder.create().texOffs(72,0).addBox(-1.2F,0,-1.2F,2.4F,6F,2.4F), PartPose.offset(2.4F,4F,0));
        PartDefinition rl=b.addOrReplaceChild("right_leg", CubeListBuilder.create().texOffs(72,0).addBox(-1.2F,0,-1.2F,2.4F,6F,2.4F), PartPose.offset(-2.4F,4F,0));
        addFoot(ll); addFoot(rl);
        return LayerDefinition.create(mesh,96,64);
    }
    private static void addFoot(PartDefinition leg){
        PartDefinition f=leg.addOrReplaceChild("foot", CubeListBuilder.create().texOffs(72,10).addBox(-2F,-.7F,-3F,4F,1.4F,5F), PartPose.offset(0,6F,0));
        for(int i=-1;i<=1;i++) f.addOrReplaceChild("talon"+(i+1), CubeListBuilder.create().texOffs(84,10).addBox(-.35F,-.35F,-3F,.7F,.7F,3F), PartPose.offset(i*1.2F,0,-2F));
    }
    ...
    @Override public void setupAnim(EagleEntity e,float limb,float amount,float age,float yaw,float pitch){
        ...
        boolean air=!e.onGround()||e.getDeltaMovement().y>.05;
        if(air){ float flap=Mth.sin(age*.55F)*.48F; leftWing.zRot=-.20F-flap; rightWing.zRot=.20F+flap; leftForeWing.zRot=-.12F-flap*.35F; rightForeWing.zRot=.12F+flap*.35F; leftWing.xRot=rightWing.xRot=-.10F; tail.xRot=.16F; leftLeg.xRot=rightLeg.xRot=.65F; }
