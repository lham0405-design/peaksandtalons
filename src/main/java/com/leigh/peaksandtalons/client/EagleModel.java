...
public class EagleModel extends HierarchicalModel<EagleEntity>{
 public static final ModelLayerLocation LAYER_LOCATION=new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(PeaksAndTalons.MOD_ID,"eagle_v4"),"main");
 ...
 public static LayerDefinition createBodyLayer(){MeshDefinition mesh=new MeshDefinition();PartDefinition r=mesh.getRoot();
  ...
  PartDefinition lw=b.addOrReplaceChild("left_wing",CubeListBuilder.create().texOffs(0,39).addBox(0,-2F,-3.5F,11F,3F,10F),PartPose.offset(4.5F,-2,-1));PartDefinition lfw=lw.addOrReplaceChild("forewing",CubeListBuilder.create().texOffs(0,52).addBox(0,-1.3F,-3F,12F,2.6F,9F),PartPose.offset(10F,0,0));
  PartDefinition rw=b.addOrReplaceChild("right_wing",CubeListBuilder.create().texOffs(0,39).mirror().addBox(-11F,-2F,-3.5F,11F,3F,10F).mirror(false),PartPose.offset(-4.5F,-2,-1));PartDefinition rfw=rw.addOrReplaceChild("forewing",CubeListBuilder.create().texOffs(0,52).mirror().addBox(-12F,-1.3F,-3F,12F,2.6F,9F).mirror(false),PartPose.offset(-10F,0,0));
  ...
  PartDefinition t=b.addOrReplaceChild("tail",CubeListBuilder.create().texOffs(34,52).addBox(-4.5F,-1F,0,9F,2F,9F),PartPose.offset(0,1,7));
 ...
 @Override public void setupAnim(EagleEntity e,float limb,float amount,float age,float yaw,float pitch){root.getAllParts().forEach(ModelPart::resetPose);head.yRot=yaw*Mth.DEG_TO_RAD;head.xRot=pitch*Mth.DEG_TO_RAD;boolean air=!e.onGround()||Math.abs(e.getDeltaMovement().y)>.02;float speed=(float)e.getDeltaMovement().horizontalDistance();
  if(air){float rate=speed>.45F?.42F:.72F;float amp=speed>.55F?.42F:1.02F;float flap=Mth.sin(age*rate)*amp;leftWing.zRot=-.12F-flap;rightWing.zRot=.12F+flap;leftForeWing.zRot=-.08F-flap*.5F;rightForeWing.zRot=.08F+flap*.5F;leftWing.yRot=rightWing.yRot=0;tail.xRot=.18F;leftLeg.xRot=rightLeg.xRot=.72F;}
  else{leftWing.yRot=-1.28F;rightWing.yRot=1.28F;leftWing.zRot=-.12F;rightWing.zRot=.12F;leftForeWing.zRot=rightForeWing.zRot=0;float walk=Mth.cos(limb*.6662F)*.65F*amount;leftLeg.xRot=walk;rightLeg.xRot=-walk;}
  if(e.getAttackAnimationTicks()>0){float s=e.getAttackAnimationTicks()/12F,k=Mth.sin(s*Mth.PI);head.xRot-=.8F*k;neck.xRot=-.4F*k;leftLeg.xRot=-1.25F*k;rightLeg.xRot=-1.25F*k;leftWing.zRot-=.3F*k;rightWing.zRot+=.3F*k;}}
